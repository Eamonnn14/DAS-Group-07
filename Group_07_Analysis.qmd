...
In this project, we aim to explore and analyze the movie dataset, focusing on various factors that might influence a movie's success. This dataset includes variables such as budget, votes, genre, and ratings. Through exploratory data analysis (EDA) and formal data analysis (FDA), we will identify significant patterns, detect outliers, and predict factors contributing to a movie's rating above 7. Our goal is to provide insights into the movie industry's dynamics and suggest what elements make a movie popular among viewers.
...
```{r}
# Detecting outliers
numeric_vars <- c("budget", "votes")

for (var in numeric_vars) {
  data[[paste0(var, "_log")]] <- log1p(data[[var]])
  Q1 <- quantile(data[[paste0(var, "_log")]], 0.25, na.rm = TRUE)
  Q3 <- quantile(data[[paste0(var, "_log")]], 0.75, na.rm = TRUE)
  IQR_value <- Q3 - Q1
  
  data[[paste0(var, "_log")]] <- ifelse(data[[paste0(var, "_log")]] < (Q1 - 2.5 * IQR_value) | data[[paste0(var, "_log")]] > (Q3 + 2.5 * IQR_value), NA, data[[paste0(var, "_log")]])
}
colSums(is.na(data[c("budget_log", "votes_log")]))
```
...
for (var in numeric_vars) {
  median_log_value <- median(data[[paste0(var, "_log")]], na.rm = TRUE)
  data[[paste0(var, "_log")]][is.na(data[[paste0(var, "_log")]])] <- median_log_value
  
  data[[var]] <- expm1(data[[paste0(var, "_log")]])
  
  data[[paste0(var, "_log")]] <- NULL
}

# Creating a new binary variable
data$above_7 <- ifelse(data$rating > 7, 1, 0)
```
# Initial data preprocessing included filling missing values (e.g., using median to fill missing values for budget and votes), use log-transformation converting data types.
